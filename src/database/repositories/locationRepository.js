...
/**
 * Handles database operations for the Location.
...
class LocationRepository {
  ...
  async update(id, data, options) {
    ...
    let record = await models.location.findByPk(id, {
      transaction,
    });
  ...
  /**
   * Finds the Location and its relations.
   *
   * @param {string} id
   * @param {Object} [options]
   */
  async findById(id, options) {
    ...
    const record = await models.location.findByPk(id, {
      include,
      transaction,
    });
  ...
  /**
   * Counts the number of Locations based on the filter.
   *
   * @param {Object} filter
   * @param {Object} [options]
   */
  ...
  /**
   * Finds the Locations based on the query.
  ...
  async findAndCountAll(
    ...
    let {
      rows,
      count,
    } = await models.location.findAndCountAll({
      where,
      include,
  ...
  /**
   * Lists the Locations to populate the autocomplete.
   * See https://sequelize.org/v5/manual/querying.html to learn how to
   * customize the query.
   *
   * @param {string} query
  ...
  /**
   * Fills an array of Location with relations and files.
   *
   * @param {Array} rows
   * @param {Object} [options]
   */
  ...
  /**
   * Fill the Location with the relations and files.
   *
   * @param {Object} record
   * @param {Object} [options]
   */
  async _fillWithRelationsAndFiles(record, options) {
    ...
    output.licationImg = await record.getLicationImg({
      transaction,
    });
